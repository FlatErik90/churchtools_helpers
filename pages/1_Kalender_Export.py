...
def appointments_to_df(appointments, header):
...
calenders = [c for c in client.calendars.list() if not c.isPrivate and c.name != "Amtsträger" and c.name != "Organisatorisches"]
...
if len(selected_calenders) > 0:
    ...
    if len(appointments) > 0:
        # print(appointments)
        df, highlight_rows = appointments_to_df(appointments, f"Monat: {start.strftime('%B')}")
    else:
        st.info("Keine Termine gefunden.")

    appointments_preview = client.calendars.appointments([c.id for c in calenders], start + datetime.timedelta(days=30), start + datetime.timedelta(days=12*30))
    appointments_preview = [a for a in appointments_preview if a.onBehalfOfPid == 107]
    if len(appointments_preview) > 0:
        df_preview, _ = appointments_to_df(appointments_preview, f"Folgende Monate")
    # st.write(appointments_preview)
    # print(appointments_preview)

if df is not None:
    output_buffer = io.BytesIO()
    dump_calendar(df, df_preview, start.strftime("%B"), "Folgende Monate", highlight_rows, output_buffer)
